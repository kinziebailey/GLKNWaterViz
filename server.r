...
getTresholdText <- function(object, parkcode, sitecode, charname){
 ...
 x <- c(getCharInfo(object,
                    parkcode = parkcode,
                    sitecode = sitecode,
                    charname = charname,
                    info = "LowerDescription"),
        getCharInfo(object,
                    parkcode = parkcode,
                    sitecode = sitecode,
                    charname = charname,
                    info = "UpperDescription"))
...
shinyServer(function(input, output, session){
  ...
  GraphOpts <- shiny::reactiveValues(Legend = TRUE,
                                     FontSize = 20,
                                     GoodColor = "Blue",
                                     BadColor = "Orange",
                                     OutColor = "Vermillion",
                                     ...
                                     LineWidth = 2,
                                     ShowHidePoint = T,
                                     FigureHorizontalScaling = 0.9,
                                     FigureVerticalScaling = 0.7)

  #### Reactive Values for Choosing Data ####
  DataOpts <- shiny::reactiveValues(Park = NA,
                                    Site = NA,
                                    Param = NA,
                                    Agg = NA,
                                    DateRange = NA,
  ...
  shiny::observeEvent(input$SeriesThreshLine,  {
    updateCheckboxInput(session = session,
                        inputId = "BoxThreshLine",
                        value = input$SeriesThreshLine)
  })

  shiny::observeEvent(input$BoxThreshLine,  {
    updateCheckboxInput(session = session,
                        inputId = "SeriesThreshLine",
                        value = input$BoxThreshLine)
  })
  ...
  shiny::observeEvent(input$BoxBy,  {
    updateRadioButtons(session = session,
                       inputId = "SummaryBoxBy",
                       selected = input$BoxBy)
  })

  shiny::observeEvent(input$SummaryBoxBy,  {
    updateRadioButtons(session = session,
                       inputId = "BoxBy",
                       selected = input$SummaryBoxBy)
  })
  ...
  ##### Depth ####
  # TimeDepth<-shiny::callModule(
  #   depthChooser
  #   ,id="TimeDepth"
  #   ,data=DataUse
  #   ,chosen=reactive(DataOpts$Depth)
  # )
  # observe({
  #   DataOpts$Depth <- TimeDepth()
  # })
  ...
  #   ,{
  #     DataOpts$Depth<-TimeDepth()
  #     ;shiny::callModule(depthChooser, id="SummaryDepth", data=DataUseMultiple, chosen=reactive(DataOpts$Depth))
  #   }
  # )
  ### Summary Controls ####
  ##### Park ####
  ...
  shiny::observeEvent(
    ScatterYears()
    ,DataOpts$Years<-ScatterYears()
  )
  ##### Depth ####
  # shiny::observeEvent(
  #   ScatterDepth()
  #   ,DataOpts$Depth<-ScatterDepth()
  # )
  # observe({
  #   DataOpts$Depth <- ScatterDepth()
  # })
...
#### Graphics Modal Control ####
  ...
  observeEvent(eventExpr = c(input$GraphicsModal, input$GraphicsModal2),
               ignoreInit = TRUE,
               showModal(modalDialog(title = "Graphics Options",
                                     footer = tagAppendAttributes(modalButton(tags$div("Close")),
                                                                  class="btn btn-primary"),
      column(12, hr()),
      column(12, h4("Figure size:"),
        # column(3,checkboxInput("Legend","Show Legend",GraphOpts$Legend)),
        column(6, sliderInput("FigureHorizontalScaling",
                              "Figure Horizontal Scaling",
                              min = 0.1,
                              max = 2,
                              value = GraphOpts$FigureHorizontalScaling,
                              step = 0.1))
        ,column(6, sliderInput("FigureVerticalScaling",
                               "Figure Vertical Scaling",
                               min = 0.1,
                               max = 2,
                               value = GraphOpts$FigureVerticalScaling,
                               step = 0.1))
      ),
      # column(12,hr()),
      column(12, h4("Figure Text:"),
        # column(3,checkboxInput("Legend","Show Legend",GraphOpts$Legend)),
        column(12, sliderInput("FontSize",
                               "Title, legend, and axis label font size",
                               min = 10,
                               max = 50,
                               value = GraphOpts$FontSize,
                               step = 1))
      ...
      # column(12,hr()),
      column(12, h4("Points:"),
        column(6, sliderInput("PointSize",
                              "Point Size",
                              min = 1,
                              max = 30,
                              value = GraphOpts$PointSize,
                              step = 1))
      ),
      # column(12,hr()),
      column(12, h4("Lines:"),
        column(6, sliderInput("LineWidth",
                              "Line Width",
                              min = 1,
                              max = 10,
                              value = GraphOpts$LineWidth,
                              step = 1))
        ,column(6, selectInput("ThColor",
                               "Threshold Line Color:",
                               choices = GraphColors$DisplayColor,
                               selected = GraphOpts$ThColor))
...
#### Housekeeping of data ####
  ...
  DataUse <- reactive({
    ...
    df <- suppressWarnings(df1 %>%
                             ...
                             ungroup()) %>%
      mutate(num_mos = length(unique(month)))
 ...
 DataUseMultiple <- reactive({
   ...
   #
    ...
    df <- suppressWarnings(combined_data %>%
                             ...
                             ungroup()) %>%
      dplyr::mutate(num_mos = length(unique(month)))
...
#### Reactive Properties ####
  ##### Thresholds ####
  Thresholds <- reactive({
    c(getCharInfo(WaterData,
                  parkcode = DataOpts$Park,
                  sitecode = DataOpts$Site,
                  charname = DataOpts$Param,
                  info = "LowerPoint"),
      getCharInfo(WaterData,
                  parkcode = DataOpts$Park,
                  sitecode = DataOpts$Site,
                  charname = DataOpts$Param,
                  info = "UpperPoint"))
...
#### Get Colors from user inputs ####
  ...
  summary <- shiny::reactive({
    ...
    data_values <- data_values1 %>%
      ...
      dplyr::mutate(Aggregation = dplyr::case_when(input$SummaryBoxBy == "month" ~ format(Date, "%b"),
                                                   input$SummaryBoxBy == "year" ~ format(Date, "%Y"),
    ...
    data_values_summary <- data_values_summary %>%
      dplyr::mutate(
        ...
        Minimum = ifelse(is.na(Minimum) | Minimum == Inf | Minimum == -Inf,
                         "Data not collected",
                         formatC(Minimum, format = "f", digits = 2)),
        Maximum = ifelse(is.na(Maximum) | Maximum == Inf | Maximum == -Inf,
                         "Data not collected",
                         formatC(Maximum, format = "f", digits = 2))) %>%
      dplyr::mutate(dplyr::across(c(Q1, Mean, Median, Q3),
                                  ~ifelse(is.na(.),
                                          "Data not collected",
                                          formatC(., format = "f",
                                                  digits = 2))))
  ...
  output$SummaryTable <-DT::renderDataTable({
    ...
    table <- table %>%
      dplyr::mutate(Aggregation = ifelse(Aggregation %in% month.abb,
                                         month.name[match(Aggregation, month.abb)],
                                         Aggregation))

    #SiteCodes to full site names
    site_codes <- NCRNWater::getSiteInfo(WaterData,
                                         parkcode = DataOpts$Park,
                                         info = "SiteCode")

    site_names <- NCRNWater::getSiteInfo(WaterData,
                                         parkcode = DataOpts$Park,
                                         info = "SiteName")

    site_info <- data.frame(SiteCode = site_codes,
                            SiteName = site_names,
    ...
    if (input$SummaryBoxBy %in% c("month", "year")) {
      ...
      summary_table <- dplyr::bind_rows(group_headers, table) %>%
        dplyr::mutate(Aggregation = factor(Aggregation,
    ...
    dt <- DT::datatable(summary_table,
                        colnames = c("Site", "Minimum", "Q1",
                                     "Mean", "Median", "Q3",
                                     "Maximum", "Standard<br>Deviation",
                                     "Site<br>Visits", "Total<br>Measurements",
                                     "Missing<br>Values"), escape = FALSE,
                       ...
                       options = list(paging = FALSE,
                                      autoWidth = TRUE,
                                      ordering = FALSE,
                                      dom = "<'dt-buttons'B>ltipr",
                                      buttons = c("copy","csv","excel"),
                                      keys = TRUE,
                                      headerCallback = JS("function(thead, data, start, end, display){",
                                                          "$(thead).find('th').css('text-align', 'center');",
                                                          "$(thead).find('th').filter(function() {
                                                          return $(this).html().trim() === 'Site'; }).css('text-align',
                                                          'left');",
                                                          ...
                                                          "});",
                                                          "}"))) %>%
      DT::formatStyle(columns = setdiff(names(summary_table), "Site"),
                      textAlign = "center") %>%
      DT::formatStyle(columns = "Site", textAlign = "left",
                      fontWeight = if(input$SummaryBoxBy %in% c("month", "year")) {
  ...
  summary_text_data <- reactive({
    ...
    char_info <- data.frame(
      Char = NCRNWater::getCharInfo(WaterData,
                                    parkcode = DataOpts$Park,
                                    info = "CharName"),
      CharName = NCRNWater::getCharInfo(WaterData,
                                        parkcode = DataOpts$Park,
                                        info = "DisplayName"),
      stringsAsFactors = FALSE)
    ...
    site_info <- data.frame(
      Site = NCRNWater::getSiteInfo(WaterData,
                                    parkcode = DataOpts$Park,
                                    info = "SiteCode"),
      SiteName = NCRNWater::getSiteInfo(WaterData,
                                        parkcode = DataOpts$Park,
                                        info = "SiteName"),
      stringsAsFactors = FALSE)
    ...
    site_visit_data <- raw_data %>%
      ...
      dplyr::summarise(text_sitevisit_mean = mean(Value, na.rm = TRUE),
                       .groups = "drop")
    ...
    summary_units <- unique(NCRNWater::getCharInfo(WaterData,
                                                   parkcode = DataOpts$Park,
                                                   charname = DataOpts$Param,
                                                   info = "Units"))
    ...
    site_list <- paste(
      sapply(1:length(text_summary_stats$Site), function(i) {
        site_name <- site_info$SiteName[match(text_summary_stats$Site[i],
                                              site_info$Site)]
        ...
        if (nrow(site_data) == 0 ||
          ...
          paste0("<li><b>", site_name, " -</b> Mean: ",
                 round(site_data$Mean, 2), " ", summary_units,
                 ", Median: ", round(site_data$Median, 2), " ",
                 summary_units, ".</li>") }
      }),
      collapse = "")
    ...
    if (nrow(highest_value) > 0 & nrow(lowest_value) > 0) {
      ...
      highest_lowest_sentence <- paste0(
        "<p>The highest ", FullParamName, " value across selected sites was ",
        round(highest_value$Maximum, 2), " ", summary_units, " at ",
        highest_value$SiteName, " in ", highest_month, " ", highest_year
        ,", while the lowest value was ", round(lowest_value$Minimum, 2), " ",
        summary_units, " at ", lowest_value$SiteName
        ," in ", lowest_month, " ", lowest_year, ".</p>")}

    #Generate text
    HTML(paste0(
      "<p><b><span style='font-size: 18px;'>Summary Report:</b></p>"
      ,"<p>The mean and median values for ", FullParamName,
      " at the selected sites and years are as follows:</p>",
 ...
 ##### thresholds #####
  ...
  MakeCorrPlot <- reactive({
    ...
    title <- paste0(NCRNWater::getParkInfo(object = WaterData,
                    ...
                    '-', DataOpts$Years[2],'\nTotal measurements: ',
                    n_measurements, ' (NAs: ', yvarname, ': ', y_nas,
                    '; ', xvarname,': ',x_nas, ')')
  ...
  hline <- function(y = 0, color = "red", dash = 'dash', size=1) {
    ...
    list(
      ...
      y0 = y,
      y1 = y,
      line = list(color = GraphOpts$ThColor,
                  dash = dash,
  ...
  MakeBoxPlot <- reactive({
    ...
    title <- paste0(NCRNWater::getParkInfo(object = WaterData,
                    ...
                    ': ', yname, '\nYears: ',DataOpts$Years[1], '-',
                    DataOpts$Years[2],'; Total measurements: ',
                    sum(summary_table$Total_Measurements),' (non-NA: ',
                    n_not_na, ', NA: ', n_na,')')
    ...
    summary_table <- summary_table %>% dplyr::mutate(
      ...
      ,Standard_Deviation = ifelse(Standard_Deviation == 'Data not collected',
                                   NA, Standard_Deviation)
    ) %>% dplyr::mutate(
      ...
      if (length(threshold) == 2){
        baseplot  %>%
          add_lines(
            ...
            ,line = list(width = GraphOpts$LineWidth,
                         dash = 'dash',
                         color = GraphOpts$ThColor) # Customize line style
            ,hovertemplate = paste0(
              "<br>Water Quality Threshold"
              ,"<br>", yname, ": ", threshold[1]
              # ,"<br>Reference: ", if(length(reference)==1){reference} else {reference[2]}
  ...
  BoxThresholdSummaryMultiple<-reactive({
    ...
    if (length(sitethresh)>0){
      paste("<p><strong>Threshold: </strong>",
            sitethresh,
  ...
  SeriesThresholdSummaryMultiple <- reactive({
    ...
    if (length(sitethresh)>0){
      paste("<p><strong>Threshold: </strong>",
            sitethresh,"<br><strong>Reference: </strong>",
            sitedetail,"</p>")
    } else {
      paste("This parameter has no water quality threshold.")
    }
  ...
  output$WaterTable <- DT::renderDataTable(
    ...
    expr = datatable(
      DataUseMultiple() %>% dplyr::select(
        ...
        Park = NCRNWater::getParkInfo(object = WaterData,
                                      parkcode = DataOpts$Park,
                                      info = "ParkLongName")
        ,Parameter = NCRNWater::getCharInfo(WaterData,
                                            parkcode = DataOpts$Park,
                                            sitecode = if(length(DataOpts$Site)>1) DataOpts$Site[1] else DataOpts$Site,
                                            charname = DataOpts$Param,
                                            info="DisplayName")
      ) %>% dplyr::select(
        Park
  ...
  exDUM <- shiny::reactive({
    ...
    for (site in DataOpts$Site) {
      ...
      # Getting characters
      mydatastructure[[site]][["LowerThreshold"]]<- NCRNWater::getCharInfo(WaterData,
                                                                           parkcode = DataOpts$Park,
                                                                           sitecode = site,
                                                                           charname = DataOpts$Param,
                                                                           info = "LowerDescription")
      mydatastructure[[site]][["UpperThreshold"]]<- NCRNWater::getCharInfo(WaterData,
                                                                           parkcode = DataOpts$Park,
                                                                           sitecode = site,
                                                                           charname = DataOpts$Param,
                                                                           info = "UpperDescription")
      mydatastructure[[site]][["LowerPoint"]]<- NCRNWater::getCharInfo(WaterData,
                                                                       parkcode = DataOpts$Park,
                                                                       sitecode = site,
                                                                       charname = DataOpts$Param,
                                                                       info = "LowerPoint")
      mydatastructure[[site]][["UpperPoint"]]<- NCRNWater::getCharInfo(WaterData,
                                                                       parkcode = DataOpts$Park,
                                                                       sitecode = site,
                                                                       charname = DataOpts$Param,
                                                                       info = "UpperPoint")
      mydatastructure[[site]][["Sitename"]]<- NCRNWater::getCharInfo(WaterData,
                                                                     parkcode = DataOpts$Park,
                                                                     sitecode = site,
                                                                     charname = DataOpts$Param,
                                                                     info = "SiteName")
      mydatastructure[[site]][["Characteristic"]]<- NCRNWater::getCharInfo(WaterData,
                                                                           parkcode = DataOpts$Park,
                                                                           sitecode = site,
                                                                           charname = DataOpts$Param,
                                                                           info = "DisplayName")
      mydatastructure[[site]][["Unit"]]<- NCRNWater::getCharInfo(WaterData,
                                                                 parkcode = DataOpts$Park,
                                                                 sitecode = site,
                                                                 charname = DataOpts$Param,
                                                                 info = "Units")
      ...
      if (is.na(mydatastructure[[site]][["LowerThreshold"]]) &&
        ...
        mydatastructure[[site]][["UpperThreshold"]] <- paste0(
          "Acceptable ", tolower(mydatastructure[[site]][["Characteristic"]]),
          " is below ", mydatastructure[[site]][["UpperPoint"]], " ",
        ...
        mydatastructure[[site]][["LowerThreshold"]] <- paste0(
          "Acceptable ", tolower(mydatastructure[[site]][["Characteristic"]]),
          " is above ", mydatastructure[[site]][["LowerPoint"]], " ",
          mydatastructure[[site]][["Unit"]], "."
        )
      }
      ...
      mydatastructure[[site]][["desc_exdf"]]<- mydatastructure[[site]][["exdf"]] %>%
        ...
        dplyr::mutate("Difference" = case_when(
          !is.na(mydatastructure[[site]][["UpperPoint"]])==TRUE &
            is.na(mydatastructure[[site]][["LowerPoint"]])==TRUE ~
          ...
          ,!is.na(mydatastructure[[site]][["UpperPoint"]])==TRUE &
            !is.na(mydatastructure[[site]][["LowerPoint"]])==TRUE &
            any(mydatastructure[[site]][["df"]]$Value >= mydatastructure[[site]][["UpperPoint"]],
                na.rm = TRUE) ~
            mydatastructure[[site]][["exdf"]]$Value - mydatastructure[[site]][["UpperPoint"]]
          ,!is.na(mydatastructure[[site]][["UpperPoint"]])==TRUE &
            !is.na(mydatastructure[[site]][["LowerPoint"]])==TRUE &
            any(mydatastructure[[site]][["df"]]$Value <= mydatastructure[[site]][["LowerPoint"]],
                na.rm = TRUE) ~
            mydatastructure[[site]][["exdf"]]$Value - mydatastructure[[site]][["LowerPoint"]]
        )) %>%
        ...
        dplyr::select("Site",
                      ...
                      "Difference",
                      "Units",
                      any_of(c("UpperThreshold", "LowerThreshold")))
      ...
      mydatastructure[[site]][["histdata"]] <- dplyr::left_join(mydatastructure[[site]][["totcount"]],
                                                                mydatastructure[[site]][["excount"]],
                                                                by = "Year")
      ...
      mydatastructure[[site]][["extext"]]<- c(
        paste0(mydatastructure[[site]][["grammar1"]],
               mydatastructure[[site]][["Characteristic"]],
               " water quality threshold among ",
               mydatastructure[[site]][["ntot"]],
               " observations at ",
               mydatastructure[[site]][["Sitename"]],
               " in ", mydatastructure[[site]][["recent_year"]], ".")
        ,paste0(mydatastructure[[site]][["grammar2"]],
                mydatastructure[[site]][["Characteristic"]],
                " water quality threshold among ",
                mydatastructure[[site]][["sum_ntot"]],
                " observations at ",
                mydatastructure[[site]][["Sitename"]],
                " since monitoring began in ",
                mydatastructure[[site]][["oldest_year"]], ".")
      )
      mydatastructure[[site]][["extext_bullets"]]<- paste0("<li>", mydatastructure[[site]][["extext"]],
                                                           "</li>", collapse = "")
      ...
      mydatastructure[[site]][["ExPoint"]]<- dplyr::case_when(
        is.na(mydatastructure[[site]][["UpperPoint"]]) == TRUE &
          !is.na(mydatastructure[[site]][["LowerPoint"]]) == TRUE ~
          mydatastructure[[site]][["LowerPoint"]]
        ,!is.na(mydatastructure[[site]][["UpperPoint"]]) == TRUE &
          is.na(mydatastructure[[site]][["LowerPoint"]]) == TRUE ~
          mydatastructure[[site]][["UpperPoint"]]
        ,!is.na(mydatastructure[[site]][["UpperPoint"]]) == TRUE &
          !is.na(mydatastructure[[site]][["LowerPoint"]]) == TRUE &
          any(mydatastructure[[site]][["exdf"]]$Value <= mydatastructure[[site]][["LowerPoint"]],
              na.rm = TRUE) &
          all(mydatastructure[[site]][["exdf"]]$Value < mydatastructure[[site]][["UpperPoint"]],
              na.rm = TRUE) ~
          mydatastructure[[site]][["LowerPoint"]]
        ,!is.na(mydatastructure[[site]][["UpperPoint"]]) == TRUE &
          !is.na(mydatastructure[[site]][["LowerPoint"]]) == TRUE &
          any(mydatastructure[[site]][["exdf"]]$Value >= mydatastructure[[site]][["UpperPoint"]],
              na.rm = TRUE) &
          all(mydatastructure[[site]][["exdf"]]$Value > mydatastructure[[site]][["LowerPoint"]],
              na.rm = TRUE) ~
          mydatastructure[[site]][["UpperPoint"]]
      ...
      mydatastructure[[site]][["histdata"]]$hover_text <- paste0(
        mydatastructure[[site]][["histdata"]]$Year, ", ",
        mydatastructure[[site]][["Characteristic"]], "\n",
        mydatastructure[[site]][["histdata"]]$ntot, " total observation(s)", "\n",
        mydatastructure[[site]][["histdata"]]$formatted_percent_ex,
        " of observations exceeding ",
        mydatastructure[[site]][["ExPoint"]], " ",
        mydatastructure[[site]][["Unit"]]
      )

      mydatastructure[[site]][["p"]]<- ggplot2::ggplot(mydatastructure[[site]][["histdata"]],
                                                       aes(x = Year,
                                                           y = percent_ex,
                                                           text = hover_text)) +
        geom_bar(stat = "identity",
                 fill = "lightgray") +
        scale_x_continuous(breaks = seq(min(mydatastructure[[site]][["histdata"]]$Year),
                                        max(mydatastructure[[site]][["histdata"]]$Year),
                                        by = 1)) +
        ylim(0, 100) +
        labs(
          title = paste0("Percent of non-NA ",
                         mydatastructure[[site]][["Characteristic"]],
                         " observations exceeding the water quality threshold at ",
                         mydatastructure[[site]][["Sitename"]],
                         "\nNon-NA observations: ",
                         mydatastructure[[site]][["non_na_obs"]], " (NAs: ",
                         mydatastructure[[site]][["total_obs"]]-mydatastructure[[site]][["non_na_obs"]], ")"),
          x = "Year",
      ...
      for (i in seq_len(nrow(mydatastructure[[site]][["histdata"]]))) {
        if(mydatastructure[[site]][["histdata"]]$nex[i] != 0) {
          alt_text <- c(alt_text,
                        paste0("<b>", mydatastructure[[site]][["histdata"]]$Year[i],
                               "</b>: ", mydatastructure[[site]][["histdata"]]$ntot[i],
                               " non-NA observation(s), ",
                               mydatastructure[[site]][["histdata"]]$formatted_percent_ex[i],
                               " exceeding ", mydatastructure[[site]][["ExPoint"]],
                               " ", mydatastructure[[site]][["Unit"]]))
        }
      }
      mydatastructure[[site]][["alt_raw"]] <- alt_text
      mydatastructure[[site]][["alt_bullets"]] <- paste0("<li>", mydatastructure[[site]][["alt_raw"]],
                                                         "</li>", collapse = "")
      mydatastructure[[site]][["alt_bullets2"]] <- paste0("<ul>", mydatastructure[[site]][["alt_bullets"]],
                                                          "</ul>")
      ...
      if (mydatastructure[[site]][["n_ex_year"]] == 1) {
        mydatastructure[[site]][["alt_text_line2"]] <- paste0("Exceedances of the water quality threshold of ",
                                                              mydatastructure[[site]][["ExPoint"]], " ",
                                                              mydatastructure[[site]][["Unit"]],
                                                              " were measured in only ",
                                                              mydatastructure[[site]][["recent_ex"]], ".")
      } else {
        mydatastructure[[site]][["alt_text_line2"]] <- paste0("Exceedances of the water quality threshold of ",
                                                              mydatastructure[[site]][["ExPoint"]], " ",
                                                              mydatastructure[[site]][["Unit"]],
                                                              " were measured first in ",
                                                              mydatastructure[[site]][["oldest_ex"]],
                                                              ", and most recently in ",
                                                              mydatastructure[[site]][["recent_ex"]], ".")
      }

      if (mydatastructure[[site]][["n_ex_year"]] != 0) {
        mydatastructure[[site]][["alt_text"]] <- paste0("<b>Figure Description:</b> Acceptable water quality measurements for ",
                                                        mydatastructure[[site]][["Characteristic"]], " at ",
                                                        mydatastructure[[site]][["Sitename"]], " have occurred between ",
                                                        mydatastructure[[site]][["oldest_year"]], " and ",
                                                        ...
                                                        mydatastructure[[site]][["highest_ex_rate"]], " in ",
...
#### USGS Functions ####
  ...
  getUSGSVal <- function(x){
    read.table(x, sep = "\t", header = T)[-1,] %>%
      select(Site = site_no, Discharge = 5)
  }

  getUSGSDaily <- function(x) {
    read.table(x, sep = "\t", header = T)[-1,]  %>%
      filter(month_nu == month(Sys.Date()) & day_nu == day(Sys.Date())) %>%
      dplyr::select(Site = site_no, P05 = p05_va, P25 = p25_va,
                    P50 = p50_va, P75 = p75_va, P95 = p95_va )
  }


  USGScut <- function(value, p05, p25, p50, p75, p95) {
    if (!is.na(p05) & value<=p05)
      return("<5th percentile")
    if ( (is.na(p05) & value <= p25) | (!is.na (p05) & value > p05 & value <= p25))
      return("5th - 25th percentile")
    if (value > p25 & value <= p50)
      return("25th - 50th percentile")
    if (value > p50 & value <= p75)
      return ("50th - 75th percentile")
    if ( (value > p75 & is.na(p95)) | (!is.na(p95) & value > p75 & value <= p95))
      return("75th - 95th percentile")
    if(!is.na(p95) & value > p95 )
  ...
  observeEvent(input$MapUSGS, ignoreInit = TRUE, {if (!DataOpts$USGSload) {
      withProgress(message="Loading USGS data, please be patient.", expr={
        USGSCodes <- read.csv("./Data/USGSSites.csv",
                              header = T, as.is = T,
                              colClasses = "character")$Code %>%
          paste(collapse=",")
        ...
        USGSdata <- USGSdata %>%
          mutate(CurrentURL = paste0("http://waterservices.usgs.gov/nwis/iv/?site=", Site,
        ...
        USGSdata <- USGSdata %>%
          left_join(map_df(USGSdata$CurrentURL,
                           possibly(getUSGSVal,
                                    otherwise = NULL)),
                    by = "Site")
        ...
        USGSdata <- USGSdata %>%
          left_join(map_df(USGSdata$StatURL,
                           possibly(getUSGSDaily,
                                    otherwise = NULL)),
                    by = "Site") %>%
          mutate(lat = lat %>% as.character %>% as.numeric(),
                 long = long %>% as.character %>% as.numeric,
                 Discharge = Discharge %>% as.numeric,
                 P05 = P05 %>% as.numeric,
                 P25 = P25 %>% as.numeric,
                 P50 = P50 %>% as.numeric,
                 P75 = P75 %>% as.numeric,
                 P95 = P95 %>% as.numeric ) %>%
          rowwise %>%
          mutate(DLevel = USGScut(Discharge, P05, P25, P50, P75, P95))
      })
 ...
 #### NPS Data ####
  NPSGeoData <- data.frame(ParkCode = getSiteInfo(WaterData,
                                                  info = "ParkCode"),
                           SiteCode = getSiteInfo(WaterData,
                                                  info = "SiteCode"),
                           ParkName = getSiteInfo(WaterData,
                                                  info = "ParkShortName"),
                           SiteName = getSiteInfo(WaterData,
                                                  info = "SiteName"),
                           latitude = getSiteInfo(WaterData,
                                                  info = "lat"),
                           longitude = getSiteInfo(WaterData,
                                                   info = "long"),
                           stringsAsFactors = F)
  ...
  CharIndex <- {getCharInfo(WaterData,
                            info = "LowerPoint") %>% is.na %>% not} | {getCharInfo(WaterData,
                                                                                   info = "UpperPoint") %>% is.na %>% not}
  NPSchars <- getCharInfo(WaterData,
                          info = "CharName")[CharIndex] %>% unique
  names(NPSchars) <- getCharInfo(WaterData,
                                 info = "DisplayName")[CharIndex] %>% unique
  output$MapChars <- renderUI( selectizeInput(inputId = "MapChar",
                                              label = "Parameter",
                                              choices = NPSchars[order(names(NPSchars))] ))
  ...
  MapColors2 <- colorFactor(palette = "viridis",
                            domain = c("<5th percentile",
                                       "5th - 25th percentile",
                                       "25th - 50th percentile",
                                       "50th - 75th percentile",
                                       "75th - 95th percentile",
                                       "> 95th percentile" ),
                            ordered = T )  # USGS percentile category for discharge

  ExceedData <- reactive({
    req(input$MapChar)
    exceed(WaterData,
           charname = input$MapChar)
  })
  ...
  #                          Network == "NETN" ~ 7)
    ...
    leaflet() %>%
      leaflet::addTiles(group = "Map",
                        urlTemplate = "http://services.arcgisonline.com/arcgis/rest/services/NatGeo_World_Map/MapServer/tile/{z}/{y}/{x}") %>%
  ...
  observe({
    ...
    req(NPSGeoData)
    updateCheckboxGroupInput(session, "MapIn",
                             choices = unique(NPSGeoData$ParkName),
                             inline = FALSE)
  })
  #Label directions
  ...
  NPSGeoData$label_dir <- directions[ (seq_len(nrow(NPSGeoData)) %%
                                         length(directions)) +1]
  NPSGeoData$xoffset <- sapply(NPSGeoData$label_dir,
                               function(dir) offset[[dir]][1])
  NPSGeoData$yoffset <- sapply(NPSGeoData$label_dir,
                               function(dir) offset[[dir]][2])
  ...
  observe({
    ...
    merge_data <- dplyr::left_join(filtered_data,
                                   filtered_exceed,
                                   by = c("SiteCode"="Site")) %>%
      dplyr::mutate(
        SiteName = case_when(
  ...
  observeEvent(
    ...
    shiny::updateCheckboxGroupInput(session, "MapIn", selected = character(0))
     leaflet::leafletProxy("WaterMap") %>%
       leaflet::setView(lng = map_values$netlon,
                        lat = map_values$netlat,
                        zoom = map_values$netzoom) %>%
       leaflet::clearGroup("Sites") %>%
       leaflet::clearGroup("NPS") %>%
  ...
  observe({
    if(input$MapUSGS){
      ...
      leafletProxy("WaterMap") %>%
      clearGroup("USGS") %>%
      addCircleMarkers(data = DataOpts$USGSdata,
                       group = "USGS",
                       layerId = DataOpts$USGSdata$Site,
                 label = DataOpts$USGSdata$Site,
                 color = MapColors2(DataOpts$USGSdata$DLevel),
                 opacity = .8,
                 fillOpacity = 0,
                 stroke = TRUE,
                 weight = 8) %>%
                 addLegend(position = "topright",
                           opacity = 1,
                           colors = MapColors2(c("<5th percentile",
                                                 "5th - 25th percentile",
                                                 "25th - 50th percentile",
                                                 "50th - 75th percentile",
                                                 "75th - 95th percentile",
                 ...
                 labels = c("<5th percentile",
                            ...
                            "> 95th percentile" ),
                 layerId = "usgsLegend",
                 title = " <svg height='15' width='20'> <circle cx='10' cy='10' r='4' stroke='black' stroke-width='3'
                            fill='transparent'/></svg>USGS: Discharge" )
    } else {
      leafletProxy("WaterMap") %>%
        clearGroup("USGS") %>%
        removeControl(layerId = "usgsLegend")}
  })


  observeEvent(input$WaterMap_marker_click,{
    MarkerClick <- input$WaterMap_marker_click
    ClickData <- if(MarkerClick$group == "NPS") {
      NPSGeoData %>%
        filter(SiteCode == MarkerClick$id)
    } else {
        DataOpts$USGSdata %>%
        filter(Site == MarkerClick$id)}
      ...
      clearPopups() %>% {
       switch(MarkerClick$group[1],
          NPS = addPopups(map =.,
            ...
            getCharInfo(WaterData,
                        parkcode = ClickData$ParkCode,
                        sitecode = ClickData$SiteCode,
                        charname = input$MapChar,
                        info = "DisplayName"),":",
            br(), round(
              100*ExceedData()[ExceedData()$Site==ClickData$SiteCode,]$Acceptable/ExceedData()[ExceedData()$Site==ClickData$SiteCode,]$Total,1),
            "% of measurements meet water quality standards",br(),br(),
            getTresholdText(WaterData,
                            ClickData$ParkCode,
                            ClickData$SiteCode,
                            input$MapChar), br(),br(),
            "<b>References:</b>",br(),
          ...
          USGS = addPopups(map =.,lat = MarkerClick$lat, lng = MarkerClick$lng,
              popup = paste("<b><a href=", ClickData$SiteURL,
                            "target='_blank'>",ClickData$Station, "</a></b>", br(),
                            "USGS: Current Discharge: ", ClickData$Discharge,
                            "cfs",br(), ClickData$DLevel, br(), br(), "<a href=",
                            ClickData$SiteURL,
                            "target='_blank'>Click here</a> to visit USGS gage website"
              )
          )
        )
    }
